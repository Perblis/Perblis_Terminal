...
// record and every consumer keeps its 15s polling interval, so a keyless backend
// (`not_configured`) or a dropped socket degrades silently to polling with no UI
// difference beyond latency (F7). Ably is pure-JS (no native module) → OTA-safe.
...
async function createClient(): Promise<AblyTypes.Realtime | null> {
  try {
    const token = await apiFetch<Record<string, unknown> & { status?: string }>("/realtime/token");
    if (token.status === "not_configured") return null; // keyless → polling stands
    const Ably = await import("ably");
    return new Ably.Realtime({
      authCallback: (_params, callback) => {
        apiFetch<AblyTypes.TokenRequest>("/realtime/token")
          .then((tr) => callback(null, tr as AblyTypes.TokenRequest))
          .catch((err: Error) => callback(err.message, null));
      },
    });
  } catch {
    return null; // any failure → polling stands
  }
}
...
/** Subscribe to a channel for the component's lifetime; no-op when degraded. */
