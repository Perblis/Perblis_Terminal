...
**Waves 0, 1, 2, 3 are complete and merged; the backend is deployed (auto-deploy from `main` to the Railway natural-cat project). Wave 4 (Hires & Money) is founder-approved and its build is COMPLETE on PR #27 (slices 4A–4F: fee engine, hire state-machine + availability, request/accept/decline/cancel with D-014, Bachs.io collect + §7.6 refunds, sweeps/handovers/disputes, payouts/reconciliation/notifications) — NOT yet merged or signed off; gated on the founder sandbox demo (needs `sandbox-api.bachs.io` egress + `BACHS_WEBHOOK_SECRET` + Railway env). Wave 5 stays gated until that sign-off.** This snapshot can lag — always reconcile against `Implementations.md` and the code itself (see Session start protocol).
