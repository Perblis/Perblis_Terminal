...
**Waves 0–5 are complete, merged, and founder-signed-off; the backend is deployed (auto-deploy from `main` to the Railway natural-cat project). Wave 4 (Hires & Money) SIGNED-OFF (2026-06-21): fee engine, hire state-machine + availability, request/accept/decline/cancel with D-014, §7.6 refunds, sweeps/handovers/disputes, payouts/reconciliation/notifications (PR #27), Paystack behind a pluggable gateway (D-018, PR #30) + callback wiring (PR #31). Wave 5 (Messaging) SIGNED-OFF (2026-06-21, PR #35): conversations, write-time contact masking, contact-unlock-after-Confirmed, Ably realtime (keyless-degraded). Wave 6 (Ops Console) is BUILT on branch `claude/focused-mccarthy-69u1ju` (not yet merged): django-otp TOTP admin 2FA, founder dashboard, verification/payout/reports/hires/users queues, dispute resolution, suspension cascade, weekly digest + reconciliation history — 476 tests green, OpenAPI unchanged (admin-only). Pending founder demo + sign-off before Wave 7. Deploy-only carry-overs: Paystack dashboard webhook URL (`/api/v1/payments/webhook`); `ABLY_API_KEY` for live realtime; for Wave 6 — enrol a TOTP device (`manage.py enroll_totp`) before flipping `ADMIN_2FA_REQUIRED` on, set `OPS_DIGEST_RECIPIENT`, and schedule Railway cron for `run_daily_reconciliation` + `run_weekly_digest`.** This snapshot can lag — always reconcile against `Implementations.md` and the code itself (see Session start protocol).
