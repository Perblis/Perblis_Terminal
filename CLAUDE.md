...
**Waves 0, 1, 2, 3 are complete and merged; the backend is deployed (auto-deploy from `main` to the Railway natural-cat project). Wave 4 (Hires & Money — state machine, fees, payments, payouts) is the next wave — GATED on explicit founder approval (not yet started).** This snapshot can lag — always reconcile against `Implementations.md` and the code itself (see Session start protocol).
