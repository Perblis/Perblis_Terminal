...
**Waves 0–4 are complete and merged; the backend is deployed (auto-deploy from `main` to the Railway natural-cat project). Wave 4 (Hires & Money) is FOUNDER-SIGNED-OFF (2026-06-21): slices 4A–4F (fee engine, hire state-machine + availability, request/accept/decline/cancel with D-014, collect + §7.6 refunds, sweeps/handovers/disputes, payouts/reconciliation/notifications, PR #27), then the payment provider was switched to Paystack behind a pluggable gateway (D-018, PR #30) + callback/redirect wiring (PR #31). Deploy-only carry-over: set the Paystack dashboard webhook URL (`/api/v1/payments/webhook`). Wave 5 (Messaging) is now UNBLOCKED but still needs explicit founder go to start.** This snapshot can lag — always reconcile against `Implementations.md` and the code itself (see Session start protocol).
