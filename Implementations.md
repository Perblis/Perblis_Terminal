...
## 2026-06-19 - Wave 4 production + Bachs sandbox validation (founder token)
- tag: CHORE
- area: prod API (`api-production-101c8`), Bachs sandbox, hires/payments tests
- summary: Founder supplied sandbox `BACHS_SECRET_KEY` and asked for a proper Wave 4 test. **Local:** `hires/` + `payments/` **128/128 pass**, **93% combined coverage** (85% gate met). **Production:** `/readyz` green, **42 OpenAPI paths**; `bachs: not_configured` on Railway (env not set). **Prod hire E2E (13/15):** supplier login ✓; live listing `019edeed-cac2-7e05-99ea-44f83645355b` ✓; hirer `nwabueze+hirer-w4-1781881704@perblis.com` ✓; hire `019ee072-0e28-7b95-962b-13cf43c482cb` (3d @ ₦95k → ₦285k) ✓; **D-014** verified both directions ✓; payment `initiated` but **stub URL** (`sandbox.bachs.invalid`) ✓; accept→cancel + decline flows ✓. **Bachs key:** authenticates (`GET /v1/accounts/balances` 200); org `Perblis` has **`payments_status: disabled`** — `POST /v1/checkouts` → 404 (both our adapter payload and Bachs-docs `pricing` format); payins at `/v1/payments/payins` works (empty).
- reason: Founder finished Wave 4 + provided Bachs sandbox key; validate before sign-off.
- change_ref: 2026-06-19 - Wave 4 Slice 4F: payouts + reconciliation + notifications (WAVE BUILD COMPLETE)
- notes: **Blockers for full payment E2E:** (1) Set `BACHS_SECRET_KEY` + `BACHS_WEBHOOK_SECRET` on Railway api+worker. (2) **Enable payments** on Bachs sandbox org (`payments_status` must not be `disabled`). (3) Confirm adapter vs live API — Bachs docs use `pricing`/`customer_email`/`checkout_url` and `GET /v1/payments/payins/{charge_id}`; our `bachs.py` still uses D-017 flat `amount` + `/payments/charges/*`. Update once a real checkout succeeds. Termii sender still pending → prod OTP fails; admin verify for new hirers.
