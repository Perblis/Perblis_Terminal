...
**Wave:** Waves 0, 1, 2, 3, **4 COMPLETE & merged to `main`**; backend deployed on Railway (auto-deploy from `main` to the **natural-cat** project). **Wave 4 (Hires & Money) is FOUNDER-SIGNED-OFF (2026-06-21).** **Wave 5 (Messaging) is now UNBLOCKED but still needs explicit founder go to start** (wave gating is binding; design.md §7). **Read `docs/waves/wave-5.md` first.** Local suite green (~373 tests; combined `hires`+`payments` cov ~88%, ≥85% gate held).
...
**Next — Wave 5 (Messaging), once founder-approved:** conversations, contact masking, Ably realtime + polling fallback. Read `docs/waves/wave-5.md` → FSD/TSD messaging sections. Wave 4's `hires`/`payments` contracts are frozen (unblocks Waves 7/8 to start in parallel too, per TSD §10).
