...
def test_email_otp_delivered_by_email():
    from accounts.services.delivery import deliver_email_otp

    deliver_email_otp(email="hirer@example.com", code="654321")
    assert len(mail.outbox) == 1
    assert "654321" in mail.outbox[0].body
    assert mail.outbox[0].to == ["hirer@example.com"]


@override_settings(TERMII_API_KEY="", RESEND_API_KEY="", DEBUG=True)
def test_phone_otp_never_emailed_in_dev(capsys):
    """The phone code goes to SMS/console only — never to email."""
    from accounts.services.delivery import deliver_phone_otp

    deliver_phone_otp(phone="+2348031234567", code="123456")
    assert len(mail.outbox) == 0  # never emailed
    assert "123456" in capsys.readouterr().out  # visible console fallback in dev


@override_settings(TERMII_API_KEY="", DEBUG=False)
def test_phone_otp_fails_loudly_in_prod_without_sms():
    """No silent fallback: prod without an SMS provider raises, never pretends."""
